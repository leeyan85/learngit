...
def write_weekly_report(filename,each_day_data,weekly_cpu_nearly_no_usage):
    ...
    no_use_server_sheetname='monthly_CPU_nearly_no_use'
    ...
    daily_chart.set_title ({'name': u'编译服务器30天使用趋势'})    #设置图表（上方）大标题
...
def generate_weekly_no_usage():
    ...
    begin_date=datetime.date(now.year,now.month,now.day)-datetime.timedelta(days=60)
...
if __name__=='__main__':
    ...
    #一周内每天的使用情况
    day_list,begin_timestamp,end_timestamp=generate_30_days_list(yesterday)
    region='CHN'
    ...
    filename='monthly_compile_server_usage_report.xlsx'
    ...
    send_mail("monthly compile server usage report",attachment=filename,sender='SEE@le.com',receiver=['yangaofeng@le.com'])
