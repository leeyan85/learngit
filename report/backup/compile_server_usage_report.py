...
def get_cpu_usage_items():
    items=models.Items.objects.filter(key_field='system.cpu.load[percpu,avg15]',hostid__name__startswith='pcnbj-cp')
    return items

def get_cpu_items():
    items=models.Items.objects.filter(key_field='system.hw.cpu[all,model]',hostid__name__startswith='pcnbj-cp')
    return items

def get_memory_items():
    items=models.Items.objects.filter(key_field='vm.memory.size[total]',hostid__name__startswith='pcnbj-cp')
    return items
...
def get_disk_items():
    ...
    used_items=models.Items.objects.filter(key_field__contains='used',
                                           name__contains='disk',
                                  hostid__name__startswith='pcnbj-cp').exclude(key_field__contains='#')
...
def analyze_cpu_usage_history_data(items,begintimestamp,endtimestamp,threashold):
    ...
    for item in items:
        ...
        if len(historys)==0: #cpu_load_gt_50的次数等于0
            no_usage=no_usage+1
        elif len(historys)>300:  #cpu_load_gt_50的次数大于300
            high_usage=high_usage+1
        else:
            low_usage=low_usage+1
        
        server_CPU_usage_dict[item.hostid.name]=[owner,interface.ip,len(historys)]
    server_CPU_usage_summary={'no_usage_server_count':no_usage,'high_usage_server_count':high_usage,'low_usage_server_count':low_usage}
...
def write_daily_report(filename,server_CPU_usage_dict,server_CPU_usage_summary):
    ...
    title=['high_usage_server_count','low_usage_server_count','no_usage_server_count']  
    ...
    daily_worksheet.write('I73',server_CPU_usage_summary['no_usage_server_count'])
    
    daily_worksheet.write('C73',u'cpu_load_gt_50_count表示24小时内，每5分钟cpu load大于50%的次数')
    daily_worksheet.write('C74',u'当cpu_load_gt_50_count>=300时，为使用率高')
    daily_worksheet.write('C75',u'当cpu_load_gt_50_count<300时，为使用率低')
    daily_worksheet.write('C76',u'当cpu_load_gt_50_count=0时，为没有使用')

    #画图
    chart = workbook.add_chart({'type': 'pie'})    #创建一个图表对象
    chart.set_size({'width': 450, 'height': 350})    #设置图表大小
    chart.set_title ({'name': u'编译服务器使用率'})    #设置图表（上方）大标题
    chart.set_y_axis({'name': 'count'})    #设置y轴（左侧）小标题 
    chart.set_x_axis({'text_axis': True})
           
    chart.add_series({
                      'categories':'=%s!$G$72:$I$72'%(sheetname),
                      'values':'=%s!$G$73:$I$73'%(sheetname),
...
if __name__=='__main__':
    ...
    send_mail(u"编译服务器使用率分布情况， 及每台编译服务器的磁盘大小及磁盘使用率",attachment='%s compile_server_usage_report.xlsx'%date,sender='yangaofeng@le.com',receiver=['supertv_cm@le.com'])
    #send_mail(u"编译服务器使用率分布情况， 及每台编译服务器的磁盘大小及磁盘使用率",attachment='%s compile_server_usage_report.xlsx'%date,sender='yangaofeng@le.com',receiver=['yangaofeng@le.com'])
