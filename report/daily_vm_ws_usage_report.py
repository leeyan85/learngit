...
def write_daily_usage_data():
    ...
    US_assigned_items,total_US_hosts,ZH_assigned_items,total_ZH_hosts=util.get_Region_ws_items()
    ...
    item=models.ScmDailyServerWsUsageSeparate(date=datetime.date(now.year,now.month,now.day),region='USA',servertotalcount=data_summary['ServerTotal'],assignedservercount=data_summary['AssignedServerCount'],servernotinusecount=data_summary['ServerNoInUseCount'],usedratio=data_summary['UsedRatio'])
    item.save()
    ...
    item=models.ScmDailyServerWsUsageSeparate(date=datetime.date(now.year,now.month,now.day),region='CHN',servertotalcount=data_summary['ServerTotal'],assignedservercount=data_summary['AssignedServerCount'],servernotinusecount=data_summary['ServerNoInUseCount'],usedratio=data_summary['UsedRatio'])
    item.save()
    ...
    item=models.ScmDailyServerWsUsageSeparate(date=datetime.date(now.year,now.month,now.day),region='All',servertotalcount=ServerTotal,assignedservercount=AssignedServerCount,servernotinusecount=ServerNoInUseCount,usedratio=UsedTatio)
    item.save()
