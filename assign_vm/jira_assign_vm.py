...
from ssh_key import *
from ansible_api_v2 import run_playbook
...
def init_gitconfig(ip,name):
    run_playbook(ip,"/letv/scripts/ansible/yml/gitconfig.yml",extra_vars={'username':name})
    ...
    for request in requests:
        ...
        if request.fields.customfield_13414 is None:
            ...
            while assigned_vm<request_VM_number:
                ...
                #Init gitconfig file
                init_gitconfig(resource_ip,owner.split('@')[0])
                #generate ssh key for gerrit
		key=generate_ssh_keys(resource_ip,owner.split('@')[0])
                #add ssh key to gerrit server
